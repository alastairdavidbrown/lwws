...
#define VERYBIG 65635 // asking fro sigger trouble
...
int main(int argv, char ** argc)
{
  ...
  struct sockaddr_in name;
    ...
    while (1)
    {
        ...
        // char sBody[BIG] = "<html><head></head><body><h1>Hello Another World!</h1></body></html>";
        // char sMsg[BIG];
        // sprintf(sMsg, "HTTP1.1 200 OK\nContent-Type: text/html\nContent-Length: %ld\n\n%s", (long) strlen(sBody), sBody);
        // LOGGER(10, "********sMsg:%s\n", sMsg);
        // int iSend = send (iAccept, sMsg, strlen(sMsg)+1, MSG_OOB);
        //
